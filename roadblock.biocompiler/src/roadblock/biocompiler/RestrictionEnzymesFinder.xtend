...
class RestrictionEnzymesFinder {
	...
	new(BiocompilerCell cell, String suppliers) {
		...
		this.databaseLocation = BinaryPathProvider.getInstance().restrictionEnzymesDbPath;
	...
	def HtmlLog searchRE() {
		...
		var numberRequested = cell.devices.map[parts.filter[biologicalFunction == 'CLONINGSITE'].size].reduce [ a, b |
			a + b
		]
		var numberPotentialRE = populatingPotentialRETable(cell, suppliers)

		if (numberRequested == 0) {
			log.addText("No restriction enzymes requested.");
			return log
		} // no RE to look for
		...
		while (updateRBS < updateRBSMax) {
			...
			numberRequested = cell.devices.map[parts.filter[biologicalFunction == 'CLONINGSITE'].size].reduce [ a, b |
				a + b
			]
			if (findFittingRE) {
				// success
				...
				numberRequested = cell.devices.map [
					parts.filter[biologicalFunction == 'CLONINGSITE' && sequence != ''].size
				].reduce[a, b|a + b]
	...
	def boolean findFittingREWithCodonOptimisation(Integer numberRequested) {
		...
		var species = "w3110"
		var cofre = new CodonOptimisationForRestrictionEnzymes(CDSList.map[utils.finalSequence(it)].toList, reList,
			species, pathToResources)
		...
		if (fittingREList.empty) {
			...
			for (sequenceUpdate : codonToUpdateList) {
				...
				CDSList.get(sequenceUpdate.partID).sequence = utils.changeSubsequenceInSequence(originalSequence,
					sequenceUpdate.sequence, sequenceUpdate.location)
			}

			// set the cloning sites to the fittingREs found
			val iteratorRE = fittingREList.iterator
			cell.devices.forEach [
				parts.filter[biologicalFunction == 'CLONINGSITE']?.filter[sequence == ''].forEach [
	...
	def boolean findFittingRE() {
		...
		val sql = db.
			prepare(
				"SELECT RE.name, RE.sequence FROM RestrictionEnzyme RE INNER JOIN PotentialRE PRE ON PRE.REid=RE.id WHERE fitsRBS=1 AND fitsCDS=1 ORDER BY LENGTH(RE.sequence)"
			)
		...
		try {
			for (cloningSite : cell.devices.map[parts.filter[biologicalFunction == 'CLONINGSITE']].flatten) {
				...
				var preSequence = cell.devices.map[parts.filter[position.value < cloningSitePosition]].flatten.sortBy [
					position.value
				].map[utils.finalSequence(it)].join
				var postSequence = cell.devices.map[parts.filter[position.value > cloningSitePosition]].flatten.sortBy [
					position.value
				].map[utils.finalSequence(it)].join
				keepSearching = true
				...
				while (sql.step && keepSearching) {
					...
					val candidateFits = utils.exactlyOneMatch(preSequence + candidateSequence + postSequence,
						candidateSequence)
					if (candidateFits) {
						keepSearching = false
						cloningSite => [
	...
	def public Integer populatingPotentialRETable(BiocompilerCell cell, String suppliers) {
		...
		val sql = db.prepare(
			"SELECT id, name, sequence FROM RestrictionEnzyme WHERE LENGTH(sequence)>cutSite AND supplier LIKE '%" +
				suppliers + "%' ORDER BY LENGTH(sequence)*ABS(RANDOM()/9223372036854775807.0)")
		while (sql.step) {
				...
				for (part : device.parts) {
					...
					if (partSequence != null) {
						if (device.direction.value == 0) {
							partSequence = utils.reverseComplement(partSequence)
						}
	...
	def List<RestrictionEnzyme> getRestrictionEnzymesList(String filter) {
		...
		var sql = db.prepare(
			"SELECT RE.name, RE.sequence FROM RestrictionEnzyme RE INNER JOIN PotentialRE PRE ON PRE.REid=RE.id WHERE " +
				filter + " ORDER BY LENGTH(RE.sequence)")
		while (sql.step) {
