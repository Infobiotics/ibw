...
//		}
		...
		for (k : 0 .. (codonList.size - 1)) {
			...
			store.impose(new Element(codonList.get(k).jCodon, codonList.get(k).costs.map [
				Math.abs((10000.0 * it)).intValue
			], codonList.get(k).jCost))
		}
		store.consistency
	...
	def findAtLeastNRestrictionEnzymes(Integer n) {
		...
		if (result) {
			println(
				...
				var fittingREIDList = fittingRestrictionEnzymes.filter[!it.fittingCombinationID.empty].filter [
					it.jRE.value == 1
				].map[it.reID].toList
				...
				var fittingCodonList = codonList.filter[jCost.value != 0].map [
					new SequenceUpdate(it.cdsID, it.position, it.forms.get(it.jCodon.value))
				].toList
			...
			def static List<FittingRestrictionEnzyme> tryAllCodonCombination(List<Codon> codonList,
				List<String> cdsList, List<RestrictionEnzyme> reList) {
...
//			println("\t" + wholeSequence)
			...
			def static computeFormsAndCostsForCodonList(List<Codon> codonList, List<String> cdsList,
				LinkedHashMap<String, CodonUsageTableElement> codonUsageTable) {
				for (codon : codonList) {
			...
			def static cumulativeProduct(List<Integer> nList) { // starts with 1 
				return nList.fold({
					var List<Integer> a = newArrayList;
					a.add(1);
					a
				}) [ a, b |
					{
			...
			def static LinkedHashMap<String, CodonUsageTableElement> prepareFormsAndCostsTable(String species,
				...
				val allAminoAcids = #['Lysine', 'Alanine', 'Glycine', 'Glutamine', 'STOP', 'Arginine', 'Glutamic acid',
					'Phenylalanine', 'Asparagine', 'Tryptophan', 'Leucine', 'Aspartic acid', 'Methionine', 'Histidine',
					'Valine', 'Cysteine', 'Isoleucine', 'Threonine', 'Proline', 'Serine', 'Tyrosine']

				for (aminoAcid : allAminoAcids) {
					var sql = db.prepare(
						"SELECT CodonUsage.codon, usage FROM CodonUsage JOIN CodonTable ON CodonTable.codon=CodonUsage.codon WHERE CodonUsage.species LIKE '%" +
							species + "%' AND aminoAcid ='" + aminoAcid + "' ORDER BY CodonUsage.codon")
			...
			def static CodonUsageTableElement computeFormsAndCosts(String codon,
				LinkedHashMap<String, CodonUsageTableElement> codonUsageTable) {
				...
				var costs = v.costs.<Double>clone
				costs.set(j, 0.0)
			...
			def static findUniqueCodons(List<String> cdsList, List<RestrictionEnzyme> reList) {
				...
				for (re : reList) {
					...
					for (cdsId : 0 .. (cdsList.size - 1)) {
						...
						if (indices.size > 0) {
							for (i : indices) {
								for (codonPosition : utils.uniqueInteger(
									(i .. (i + re.sequence.length - 1)).map[it / 3].toList)) {
			...
			def static List<Codon> uniqueCodons(List<Codon> cl) { // returns the unique elements
				cl.fold(<Codon>newArrayList) [ a, b |
					if (a.exists[(it as Codon).cdsID == b.cdsID && (it as Codon).position == b.position])
						a
					else {
						a.add(b);
						a
